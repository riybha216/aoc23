...
def get_if_valid_part(symbols, tracker, mat):
...
def process_line(mat):
    ...
    for i in range(len(mat)):
        for j in range(len(mat[0])):
            if mat[i][j] != '.' and not(mat[i][j].isnumeric()):
                symbol_locs.add((i, j))
    ...
    for i in range(len(mat)):
        for j in range(len(mat[0])):
            if mat[i][j].isnumeric():
                ...
                valid = get_if_valid_part(symbol_locs, tracker, mat)
                if valid:
                    total += list2int(num)
                num.clear()
                tracker.clear()
